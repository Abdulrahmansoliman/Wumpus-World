...
class WumpusApp:
    ...
    def __init__(self) -> None:
        ...
        self.mode_var = tk.StringVar(value="prolog")
        self.reveal_var = tk.BooleanVar(value=True)
        self.risk_var = tk.DoubleVar(value=0.10)  # epsilon for Q-learning
    ...
    def _build_layout(self) -> None:
        ...
        tk.OptionMenu(controls, self.mode_var, "prolog", "qlearn", "human").grid(row=0, column=1, sticky="we", pady=(6, 0))
        ...
        tk.Label(controls, text="Riskiness ε", fg="#e2e8f0", bg="#0f172a").grid(row=2, column=0, sticky="w", pady=(6, 0))
        tk.Scale(
            controls,
            ...
            command=lambda _v: self._apply_riskiness(),
        ...
        start_btn.grid(row=3, column=0, columnspan=3, sticky="we", pady=(6, 0))
        ...
        legend_row("#e2e8f0", "B/S/G/GL = Breeze/Stench/Glitter/Glimmer")
    ...
    def prompt_and_start_game(self) -> None:
        ...
        size_var = tk.IntVar(value=self.grid_size_var.get())
        seed_var = tk.StringVar(value=self.seed_var.get())
    ...
    def start_game(self) -> None:
        ...
        if mode == "prolog":
            ...
            model_path = Path("models/q_hybrid.pkl")
            if not model_path.exists():
                messagebox.showerror(
                    "Missing model", "Train Q-learning first: run qlearn_train.py to create models/q_hybrid.pkl"
                )
                self.running = False
                return
            payload = QLearningAgent.load(model_path)
            cfg = QConfig(alpha=payload["alpha"], gamma=payload["gamma"], epsilon=float(self.risk_var.get()))
            self.agent = QLearningAgent(grid_size=size, config=cfg, qtable=payload["q"])
            self.agent.reset()
            self._apply_riskiness()
        self.current_percept = self.world.initial_percept()
        self.visited_percepts = {self.world.agent_pos: self.current_percept}
        self.step_count = 0
        self.running = True
        self._set_buttons_enabled(mode == "human")
        self._log("New game started.")
        self._update_status()
        self._draw_world()
        if mode in ("prolog", "qlearn") and self.agent:
    ...
    def _apply_riskiness(self) -> None:
        if self.agent is None:
            return
        if hasattr(self.agent, "set_epsilon"):
            try:
                self.agent.set_epsilon(float(self.risk_var.get()))
            except Exception:
                pass
    ...
    def _update_status(self, result: Optional[object] = None) -> None:
        ...
        if self.current_percept:
            ...
            percept_str = (
                ...
                f"Glimmer={p.glimmer}, Bump={p.bump}, Scream={p.scream}"
            )
