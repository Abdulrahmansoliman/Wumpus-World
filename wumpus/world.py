...
class Percept:
    ...
    stench: bool = False
    glitter: bool = False
    glimmer: bool = False  # Gold is in a neighboring cell
    bump: bool = False
    scream: bool = False
...
class World:
    ...
    def is_solvable(self) -> bool:
        """Check if there's a path from start to gold and back, avoiding pits.
        
        Considers that wumpus can be killed with arrow, so only pits block permanently.
        """
        from collections import deque
        
        # Obstacles that permanently block (pits only - wumpus can be killed)
        blocked = self.pits
        
        def can_reach(start: Position, goal: Position) -> bool:
            if start == goal:
                return True
            queue = deque([start])
            visited = {start}
        ...
        # Must be able to reach gold AND return home
        can_reach_gold = can_reach(self.start, self.gold_pos)
        can_return_home = can_reach(self.gold_pos, self.start)
        
        return can_reach_gold and can_return_home
    ...
    def _compute_percept(self, bump: bool, scream: bool) -> Percept:
        ...
        # Glimmer: gold is in a neighboring cell (not picked up yet)
        glimmer = (not self.has_gold) and any(
            n == self.gold_pos for n in self.neighbors(self.agent_pos)
        )
        return Percept(breeze=breeze, stench=stench, glitter=glitter, glimmer=glimmer, bump=bump, scream=scream)
